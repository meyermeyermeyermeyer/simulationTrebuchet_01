...
% --- Condition initiale ---
y0_sim = 4;         % Hauteur initiale de lancement (m)
...
% Extraction de vx et vy au moment du déploiement
if idx_para > 1
...
hold on;
yline(Pa_CdAfull, '--k', sprintf('CdA_{deploye} = %.4f m²', Pa_CdAfull), ...
...
ylabel('C_d \times A (m²)');
title('Evolution du C_d \times A — inflation de Pa\_CdA0 vers Pa\_CdAfull');
xlim([0 Pa_tspan(2)]);
...
function [y_max, x_final, X, Y, T, V_mag, A_mag, idx_para] = sim_physique_complete( ...
    ...
    Pa_tau, Pa_CdA0, Pa_CdAfull, Pa_m, ...
    y0, rho, g, dt)
    ...
    while y >= 0
        ...
        if t < t_para
            ...
            %-- Phase parachute : traînée vectorielle complète --
            if idx_para == 0
                idx_para = length(X);
            end
            ...
            % CdA effectif : part de Pa_CdA0, monte vers Pa_CdAfull
            CdA_eff = Pa_CdA0 + (Pa_CdAfull - Pa_CdA0) * (1 - exp(-t_dep_loc / Pa_tau));
 
            % ✅ CORRECTION : drag appliqué aux deux composantes via la norme de v
            v_loc     = sqrt(vx^2 + vy^2);
            drag_coef = 0.5 * rho * CdA_eff * v_loc / Pa_m;
            ax        = -drag_coef * vx;
            ay        = -drag_coef * vy - g;
        end
...
function dxdt = Pa_dyn_precise( ...
    ...
    % ✅ CORRECTION : drag vectoriel appliqué aux deux composantes
    v_mag  = sqrt(vx_loc^2 + vy_loc^2);
    drag_c = 0.5 * rho_loc * CdA_loc * v_mag / Pa_m_loc;
    ax_loc = -drag_c * vx_loc;
    ay_loc = -drag_c * vy_loc - g_loc;
...
function [ax_out, ay_out] = Pa_accel_precise( ...
    ...
    v_mag  = sqrt(vx_loc^2 + vy_loc^2);
    drag_c = 0.5 * rho_loc * CdA_loc * v_mag / Pa_m_loc;
    ax_out = -drag_c * vx_loc;
    ay_out = -drag_c * vy_loc - g_loc;
end